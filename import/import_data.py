...
def main():
    ...
    print("starting database connection to %s", dbname)
...
fac_code=(%s)), (%s));''', (a_pk, transfer[6], transfer[8]))
    conn.commit()
    ...
    for asset in assets_info:
        if asset[4] != 'NULL':
            ...
            cur.execute('''UPDATE asset_history SET depart_dt=(%s) WHERE 
asset_fk = (SELECT asset_pk FROM assets WHERE asset_tag=(%s)) AND depart_dt IS 
NULL AND facility_fk IS NOT NULL;''', (asset[0],))
            #set disposal entry
            cur.execute('''INSERT INTO asset_history (asset_fk, arrive_dt) 
VALUES ((SELECT asset_pk FROM assets WHERE asset_tag=(%s)), (%s));''', (asset[0], asset[4]))
    conn.commit()
